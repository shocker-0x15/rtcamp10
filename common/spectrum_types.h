...
struct SampledSpectrumTemplate {
    ...
    CUDA_DEVICE_FUNCTION CUDA_INLINE static constexpr SampledSpectrumTemplate Zero() {
        return SampledSpectrumTemplate(0.0);
    }
    CUDA_DEVICE_FUNCTION CUDA_INLINE static constexpr SampledSpectrumTemplate One() {
        return SampledSpectrumTemplate(1.0);
    }
    CUDA_DEVICE_FUNCTION CUDA_INLINE static constexpr SampledSpectrumTemplate Inf() {
        return SampledSpectrumTemplate(INFINITY);
    }
    CUDA_DEVICE_FUNCTION CUDA_INLINE static constexpr SampledSpectrumTemplate NaN() {
        return SampledSpectrumTemplate(NAN);
    }
};

template <typename RealType, uint32_t NumSpectralSamples>
CUDA_DEVICE_FUNCTION CUDA_INLINE constexpr SampledSpectrumTemplate<RealType, NumSpectralSamples>
    min(const SampledSpectrumTemplate<RealType, NumSpectralSamples> &vA,
        const SampledSpectrumTemplate<RealType, NumSpectralSamples> &vB) {
    SampledSpectrumTemplate<RealType, NumSpectralSamples> ret;
    for (int i = 0; i < NumSpectralSamples; ++i)
        ret[i] = std::fmin(vA[i], vB[i]);
    return ret;
}

template <typename RealType, uint32_t NumSpectralSamples>
CUDA_DEVICE_FUNCTION CUDA_INLINE constexpr SampledSpectrumTemplate<RealType, NumSpectralSamples>
    min(const SampledSpectrumTemplate<RealType, NumSpectralSamples> &value, RealType minValue) {
    SampledSpectrumTemplate<RealType, NumSpectralSamples> ret;
    for (int i = 0; i < NumSpectralSamples; ++i)
...
template <typename RealType, uint32_t NumSpectralSamples>
CUDA_DEVICE_FUNCTION CUDA_INLINE constexpr SampledSpectrumTemplate<RealType, NumSpectralSamples>
    max(const SampledSpectrumTemplate<RealType, NumSpectralSamples> &value, RealType maxValue) {
    SampledSpectrumTemplate<RealType, NumSpectralSamples> ret;
    for (int i = 0; i < NumSpectralSamples; ++i)
...
template <typename RealType, uint32_t NumSpectralSamples>
CUDA_DEVICE_FUNCTION CUDA_INLINE constexpr SampledSpectrumTemplate<RealType, NumSpectralSamples>
    lerp(const SampledSpectrumTemplate<RealType, NumSpectralSamples> &v0,
            const SampledSpectrumTemplate<RealType, NumSpectralSamples> &v1, RealType t) {
    return (1 - t) * v0 + t * v1;
}
...
private:
    ...
    CUDA_DEVICE_FUNCTION void interpolateCoefficients(
        RealType e0, RealType e1, RealType e2, const PolynomialCoefficients* table);
#endif
...
public:
    ...
    CUDA_DEVICE_FUNCTION CUDA_INLINE static constexpr uint32_t GridWidth() { return 12; }
    CUDA_DEVICE_FUNCTION CUDA_INLINE static constexpr uint32_t GridHeight() { return 14; }
...
#   endif
    ...
    CUDA_DEVICE_FUNCTION CUDA_INLINE static constexpr RealType EqualEnergyReflectance() {
        return static_cast<RealType>(0.009355121400914532);
    }

    CUDA_DEVICE_FUNCTION CUDA_INLINE static constexpr void xy_to_uv(const RealType xy[2], RealType uv[2]) {
        uv[0] =
    ...
    CUDA_DEVICE_FUNCTION CUDA_INLINE static constexpr void uv_to_xy(const RealType uv[2], RealType xy[2]) {
        xy[0] =
...
public:
    CUDA_DEVICE_FUNCTION RegularSampledSpectrumTemplate(
        RealType minLambda, RealType maxLambda, const RealType* values, uint32_t numSamples) :
...
public:
    CUDA_DEVICE_FUNCTION IrregularSampledSpectrumTemplate(
        const RealType* lambdas, const RealType* values, uint32_t numSamples) :
...
public:
    ...
    CUDA_DEVICE_FUNCTION constexpr explicit DiscretizedSpectrumTemplate(const RealType* vals) : 
    ...
    template <uint32_t N>
    CUDA_DEVICE_FUNCTION constexpr DiscretizedSpectrumTemplate(
        const WavelengthSamplesTemplate<RealType, N> &wls,
    ...
    CUDA_DEVICE_FUNCTION RealType &operator[](unsigned int index) {
        Assert(index < NumStrataForStorage, "\"index\" is out of range [0, %u].", NumStrataForStorage - 1);
        return values[index];
    }
    CUDA_DEVICE_FUNCTION RealType operator[](unsigned int index) const {
        Assert(index < NumStrataForStorage, "\"index\" is out of range [0, %u].", NumStrataForStorage - 1);
    ...
    template <uint32_t N>
    CUDA_DEVICE_FUNCTION DiscretizedSpectrumTemplate &add(
    ...
    CUDA_DEVICE_FUNCTION CUDA_INLINE static constexpr uint32_t NumStrata() {
        return NumStrataForStorage;
    }
    CUDA_DEVICE_FUNCTION CUDA_INLINE static constexpr DiscretizedSpectrumTemplate Zero() {
        return DiscretizedSpectrumTemplate(0.0f);
    }
    CUDA_DEVICE_FUNCTION CUDA_INLINE static constexpr DiscretizedSpectrumTemplate One() {
        return DiscretizedSpectrumTemplate(1.0f);
    }
    CUDA_DEVICE_FUNCTION CUDA_INLINE static constexpr DiscretizedSpectrumTemplate Inf() {
        return DiscretizedSpectrumTemplate(INFINITY);
    }
    CUDA_DEVICE_FUNCTION CUDA_INLINE static constexpr DiscretizedSpectrumTemplate NaN() {
        return DiscretizedSpectrumTemplate(NAN);
    }
    ...
    CUDA_DEVICE_FUNCTION void print() const {
        devPrintf("%g", values[0]);
        for (int i = 1; i < NumStrataForStorage; ++i)
            devPrintf(", %g", values[i]);
        devPrintf("\n");
    }
};
...
public:
    ...
    template <uint32_t N>
    CUDA_DEVICE_FUNCTION SpectrumStorageTemplate &add(
        const WavelengthSamplesTemplate<RealType, N> &wls, const SampledSpectrumTemplate<RealType, N> &val) {
        const RealType recBinWidth = NumStrataForStorage / (WavelengthHighBound - WavelengthLowBound);
