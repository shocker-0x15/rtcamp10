...
static void findBestObjectSplit(
    ...
    for (uint32_t primRefIdx = 0; primRefIdx < numPrimRefs; ++primRefIdx) {
        ...
        const auto binIdx3D = make_uint3(
            std::min<uint32_t>(numObjBins * np.x, numObjBins - 1),
            std::min<uint32_t>(numObjBins * np.y, numObjBins - 1),
            std::min<uint32_t>(numObjBins * np.z, numObjBins - 1));
        for (int32_t dim = 0; dim < 3; ++dim) {
            //const uint32_t binIdx = binIdx3D[dim];
            const uint32_t binIdx = getComp(binIdx3D, dim);
...
static void findBestSpatialSplit(
    ...
    for (uint32_t primRefIdx = 0; primRefIdx < numPrims; ++primRefIdx) {
        ...
        const auto entryBinIdx3D = make_uint3(
            std::min<uint32_t>(numSpaBins * entryNp.x, numSpaBins - 1),
            std::min<uint32_t>(numSpaBins * entryNp.y, numSpaBins - 1),
            std::min<uint32_t>(numSpaBins * entryNp.z, numSpaBins - 1));
        const auto exitBinIdx3D = make_uint3(
            std::min<uint32_t>(numSpaBins * exitNp.x, numSpaBins - 1),
            std::min<uint32_t>(numSpaBins * exitNp.y, numSpaBins - 1),
            std::min<uint32_t>(numSpaBins * exitNp.z, numSpaBins - 1));
        for (int32_t dim = 0; dim < 3; ++dim) {
            //const uint32_t entryBinIdx = entryBinIdx3D[dim];
            //const uint32_t exitBinIdx = exitBinIdx3D[dim];
            const uint32_t entryBinIdx = getComp(entryBinIdx3D, dim);
            const uint32_t exitBinIdx = getComp(exitBinIdx3D, dim);
